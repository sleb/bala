...
///
/// `Delete` is the `dd` sequence; `CompleteCascade` is the confirmation
/// for completing a task that has incomplete children (which completes
/// them too). `InheritFromParent` is the prompt shown after creating a new
